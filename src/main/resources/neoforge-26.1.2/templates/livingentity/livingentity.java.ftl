...
public class ${name}Entity extends ${extendsClass} <#if interfaces?size gt 0>implements ${interfaces?join(",")}</#if> {
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		<#if data.spawnThisMob>
			...
			<#assign customPlacementMode = "PRESET">
			<#assign customPlacementPreset = "">
			<#assign customPlacementType = "ON_GROUND">
			<#assign customHeightmap = "MOTION_BLOCKING_NO_LEAVES">
			<#assign customSpawnCondition = "AMBIENT">
			<#assign hasCategorySpawnProcedure = false>
			<#if selectedSpawnType?starts_with("CUSTOM:")>
				<#assign selectedCategoryName = selectedSpawnType?remove_beginning("CUSTOM:")>
				<#assign selectedCategoryElement = w.getWorkspace().getModElementByName(selectedCategoryName)>
				<#if selectedCategoryElement??>
					<#assign selectedCategoryDefinition = selectedCategoryElement.getGeneratableElement()>
					<#if selectedCategoryDefinition??>
						<#if selectedCategoryDefinition.placementMode?? && selectedCategoryDefinition.placementMode?has_content>
							<#assign customPlacementMode = selectedCategoryDefinition.placementMode>
						</#if>
						<#if selectedCategoryDefinition.placementPreset?? && selectedCategoryDefinition.placementPreset?has_content>
							<#assign customPlacementPreset = selectedCategoryDefinition.placementPreset>
						</#if>
						<#if selectedCategoryDefinition.customPlacementType?? && selectedCategoryDefinition.customPlacementType?has_content>
							<#assign customPlacementType = selectedCategoryDefinition.customPlacementType>
						</#if>
						<#if selectedCategoryDefinition.customHeightmap?? && selectedCategoryDefinition.customHeightmap?has_content>
							<#assign customHeightmap = selectedCategoryDefinition.customHeightmap>
						</#if>
						<#if selectedCategoryDefinition.customSpawnCondition?? && selectedCategoryDefinition.customSpawnCondition?has_content>
							<#assign customSpawnCondition = selectedCategoryDefinition.customSpawnCondition>
						</#if>
						<#if selectedCategoryDefinition.spawnConditionProcedure?? && hasProcedure(selectedCategoryDefinition.spawnConditionProcedure)>
							<#assign categorySpawnConditionProcedure = selectedCategoryDefinition.spawnConditionProcedure>
							<#assign hasCategorySpawnProcedure = true>
						</#if>
					</#if>
				</#if>
			</#if>

			<#if selectedSpawnType?starts_with("CUSTOM:") && customPlacementMode == "CUSTOM">
				<#assign customPlacementTypeCode = "SpawnPlacementTypes.ON_GROUND">
				<#if customPlacementType == "IN_WATER">
					<#assign customPlacementTypeCode = "SpawnPlacementTypes.IN_WATER">
				<#elseif customPlacementType == "NO_RESTRICTIONS">
					<#assign customPlacementTypeCode = "SpawnPlacementTypes.NO_RESTRICTIONS">
				</#if>

				<#assign customHeightmapCode = "Heightmap.Types.MOTION_BLOCKING_NO_LEAVES">
				<#if customHeightmap == "MOTION_BLOCKING">
					<#assign customHeightmapCode = "Heightmap.Types.MOTION_BLOCKING">
				<#elseif customHeightmap == "OCEAN_FLOOR">
					<#assign customHeightmapCode = "Heightmap.Types.OCEAN_FLOOR">
				<#elseif customHeightmap == "WORLD_SURFACE">
					<#assign customHeightmapCode = "Heightmap.Types.WORLD_SURFACE">
				</#if>

			event.register(${JavaModName}Entities.${REGISTRYNAME}.get(),
					${customPlacementTypeCode}, ${customHeightmapCode},
					...
					<#elseif hasCategorySpawnProcedure>
					(entityType, world, reason, pos, random) -> {
						int x = pos.getX();
						int y = pos.getY();
						int z = pos.getZ();
					...
					<#elseif customSpawnCondition == "ALWAYS_ALLOW">
			...
			<#elseif selectedSpawnType == "creature" || customPlacementPreset == "CREATURE">
			event.register(${JavaModName}Entities.${REGISTRYNAME}.get(),
			...
			<#elseif selectedSpawnType == "ambient" || selectedSpawnType == "misc" || customPlacementPreset == "AMBIENT" || customPlacementPreset == "NO_RESTRICTIONS">
			event.register(${JavaModName}Entities.${REGISTRYNAME}.get(),
			...
			<#elseif selectedSpawnType == "waterCreature" || selectedSpawnType == "waterAmbient" || customPlacementPreset == "WATER_CREATURE">
			event.register(${JavaModName}Entities.${REGISTRYNAME}.get(),
			...
			<#elseif selectedSpawnType == "undergroundWaterCreature">
			event.register(${JavaModName}Entities.${REGISTRYNAME}.get(),
					SpawnPlacementTypes.IN_WATER, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
					<#if hasProcedure(data.spawningCondition)>
					(entityType, world, reason, pos, random) -> {
