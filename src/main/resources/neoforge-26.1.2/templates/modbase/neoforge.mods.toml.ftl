...
<#if w.getGElementsOfType('livingentity')?filter(e -> e.mobBehaviourType == "Raider")?size != 0 || w.hasElementsOfType('spawncategory')>
...
<#assign hasProvidedMixins = false>
<#list w.getWorkspace().getModElements() as element>
  <#if element.getGeneratableElement().mixins??>
    <#if element.getGeneratableElement().mixins?has_content>
      <#assign hasProvidedMixins = true>
    </#if>
  </#if>
</#list>
<#if w.getGElementsOfType('biome')?filter(e -> e.spawnBiome || e.spawnInCaves || e.spawnBiomeNether)?size != 0 || hasProvidedMixins>
[[mixins]]
